...
export default function TasksView() {
  ...
  // Status messages fade on their own after a few seconds so the form
  // doesn't accumulate stale "Task added" text as the person keeps working.
  React.useEffect(() => {
    if (!feedback) return;
  ...
  const handleCreateTask = async (event) => {
    ...
    if (!draft.title.trim()) {
      setFeedback({
        type: "error",
        message: "Add a short task title before saving.",
      });
      return;
    }
    ...
    if (result?.success) {
      ...
      setFeedback({ type: "success", message: "Task added to your focus board." });
    } else {
      setFeedback({
        type: "error",
        message: result?.error || "Could not save the task.",
      });
    }
  };
      ...
      <div className="rounded-2xl border border-slate-200/80 bg-white/80 p-5 shadow-sm dark:border-slate-800/80 dark:bg-slate-900/60">
        ...
        <form onSubmit={handleCreateTask} className="mt-4 space-y-3">
          <label className="flex flex-col text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">
            ...
            <input
              ...
              required
              aria-required="true"
              className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 shadow-sm dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200 focus:border-brand-400 focus:ring-2 focus:ring-brand-400/20 transition-colors"
            />
          </label>
        ...
        {feedback && (
          <div
            role="status"
            aria-live="polite"
            className={`mt-3 flex items-start justify-between gap-3 rounded-lg border px-3 py-2 text-sm animate-fadeIn ${
              feedback.type === "error"
          ...
          >
            <span className="flex items-center gap-2">
              <DynamicIcon
                name={feedback.type === "error" ? "x" : "check-circle"}
                ...
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      ...
                      className={`rounded-lg border px-3 py-2 text-[11px] font-semibold transition-all ${activeActivity && activeActivity.taskId === task.id ? "border-emerald-300 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 dark:border-emerald-900/50 dark:bg-emerald-950/40 dark:text-emerald-300" : "border-slate-200 text-slate-600 hover:bg-slate-100 dark:border-slate-800 dark:text-slate-300 dark:hover:bg-slate-800"}`}
                    >
                      {activeActivity && activeActivity.taskId === task.id
                        ? "Stop activity"
                        : "Start activity"}
                    </button>
                    <button
                      ...
                      className={`rounded-lg border px-3 py-2 text-[11px] font-semibold transition-all ${
                        pendingDeleteId === task.id
                          ...
                          : "border-rose-200 text-rose-600 hover:bg-rose-50 dark:border-rose-900/50 dark:text-rose-300 dark:hover:bg-rose-950/40"
                      }`}
                    >
                      {pendingDeleteId === task.id ? "Confirm delete?" : "Delete"}
                    </button>
                  </div>
                </div>
