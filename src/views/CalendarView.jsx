...
export default function CalendarView() {
  ...
  const selectDay = (date) => {
    if (!date) return;
    const matchLogs = activities.filter((a) => new Date(a.startTime).toDateString() === date.toDateString());
    setSelectedDateLog({
    ...
    <div className="space-y-6 animate-fadeIn">
      <div>
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400">
          Color intensity representation of completed hours mapped to calendar grids.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="grid grid-cols-7 gap-2 text-center">
            ...
            {currentMonthDays.map((date, idx) => {
              ...
              if (hours > 0 && hours <= 2)
                ...
                heatBg = "bg-brand-600 text-white border-brand-700 font-bold shadow-md shadow-brand-500/15";
              ...
              return (
                ...
                >
                  ...
                  {hours > 0 && <span className="text-[8px] opacity-85 mt-1 font-mono">{hours.toFixed(1)}h</span>}
                </button>
              );
            })}
          </div>
        ...
        <div className="p-6 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-900 shadow-sm space-y-4">
          <h3 className="text-sm font-bold text-slate-800 dark:text-slate-300">Daily Log Inspector</h3>
          {selectedDateLog ? (
            <div className="space-y-3">
              ...
              <div className="space-y-2.5 max-h-96 overflow-y-auto pr-1">
                {selectedDateLog.logs.length === 0 ? (
                  <p className="text-xs text-slate-500 italic">No operations logged on this date.</p>
                ) : (
                  selectedDateLog.logs.map((log) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-semibold text-slate-800 dark:text-slate-200 truncate">{log.title}</span>
