...
export default function ExportView() {
  ...
  const filteredActivities = useMemo(() => {
    ...
    if (exportPreset === "today") {
      ...
      temp = temp.filter((a) => new Date(a.startTime).toDateString() === todayStr);
    } else if (exportPreset === "yesterday") {
      const yesterday = new Date();
      yesterday.setDate(yesterday.getDate() - 1);
      const yesterdayStr = yesterday.toDateString();
      temp = temp.filter((a) => new Date(a.startTime).toDateString() === yesterdayStr);
    } else if (exportPreset === "week") {
      const limit = new Date();
      limit.setDate(limit.getDate() - 7);
      temp = temp.filter((a) => new Date(a.startTime) >= limit);
      ...
      if (startDate) {
        ...
        temp = temp.filter((a) => new Date(a.startTime) >= startLimit);
      }
      if (endDate) {
        const endLimit = new Date(endDate);
        endLimit.setHours(23, 59, 59, 999);
        temp = temp.filter((a) => new Date(a.startTime) <= endLimit);
      }
    }
    return temp;
    ...
    activitiesList.forEach((act) => {
      ...
      if (act.category) {
        if (!categoryMap[act.category]) {
          categoryMap[act.category] = { minutes: 0, color: act.color || "#6366f1" };
        }
  ...
  const generateCSV = () => {
    ...
    let csv = "data:text/csv;charset=utf-8,ID,Date,Start Time,End Time,Category,Deliverable / Activity,Project,Duration (Mins),Notes\n";
    ...
    filteredActivities.forEach((act) => {
      ...
      const formattedEnd = act.endTime ? new Date(act.endTime).toLocaleTimeString() : "Active";
    ...
    link.setAttribute("download", `ShiftFlow_Filtered_Timesheet_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  const generateExcel = async () => {
    ...
    filteredActivities.forEach((act) => {
      ...
      const formattedDate = actDate.toLocaleDateString([], { year: "numeric", month: "2-digit", day: "2-digit" });
      const formattedStart = actDate.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
    ...
    summarySheet.addRow(["EXECUTIVE SHIFT WORKSPACE SUMMARY", "VALUE / OUTPUT DETAILS"]);
    summarySheet.addRow(["Total Cumulative Logged Duration", `${metrics.totalHours} Hours`]);
    summarySheet.addRow(["Net Operational Focus Delivery", `${metrics.workHours} Hours`]);
    summarySheet.addRow(["Operational Break / Recharge Time", `${metrics.breakHours} Hours`]);
    summarySheet.addRow(["Timeline Segments Processed", `${metrics.activityCount} Activities`]);
    summarySheet.addRow([
    ...
    summarySheet.eachRow((row, rowNum) => {
      ...
      row.getCell(1).font = { bold: true, name: "Segoe UI", size: 10, color: { argb: "475569" } };
    });
    ...
    chartSheet.addRow(["Operational Category", "Total Duration (Mins)", "Shift Share (%)", "Visual Allocation Chart Bar"]);
    ...
    chartSheet.eachRow((row, rowNum) => {
      ...
      row.getCell(4).font = { name: "Courier New", bold: true, color: { argb: "10B981" }, size: 10 };
    });
    ...
    try {
      ...
      filteredActivities.forEach((act) => {
        ...
        const end = act.endTime ? new Date(act.endTime) : new Date(start.getTime() + Number(act.duration || 0) * 60000);
      ...
      const tmpChart = new Chart(ctx, {
        ...
        options: {
          ...
          plugins: {
            legend: { display: true, labels: { font: { family: "Segoe UI", size: 11, weight: "bold" } } },
          },
          scales: {
            y: {
              beginAtZero: true,
              max: 60,
              title: { display: true, text: "Active Operations (Minutes)", font: { family: "Segoe UI", size: 10 } },
            },
            x: { title: { display: true, text: "Hour of Day (24h format)", font: { family: "Segoe UI", size: 10 } } },
          },
        },
      });

      const imgBase = canvas.toDataURL("image/png");
      const imageId = workbook.addImage({ base64: imgBase.split(",")[1], extension: "png" });
    ...
    filteredActivities.forEach((act) => {
      ...
      const endIndex = Math.max(0, Math.min(47, Math.ceil(endMinutes / 30) - 1));

      const rowVals = [];
      const formattedStart = start.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
      const formattedEnd = end ? end.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : "Active";
      rowVals.push(`${formattedStart} - ${formattedEnd}  ${act.title}`);
      for (let i = 0; i < 48; i++) rowVals.push("");
      const row = daySheet.addRow(rowVals);

      const colorHex = act.color && act.color.startsWith("#") ? act.color : act.color || "#6366f1";
    ...
    if (metrics.categories && metrics.categories.length > 0) {
      metrics.categories.forEach((cat, idx) => {
        ...
        colorCell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: hexToARGB(cat.color || "#6366f1") } };
      });
    }
    ...
    const titleRow = memoSheet.addRow(["SHIFTFLOW COMPLIANT WORKSPACE BRIEFING REPORT"]);
    titleRow.getCell(1).font = { bold: true, size: 13, color: { argb: "4F46E5" }, name: "Segoe UI" };

    memoSheet.addRow([]);

    const assessmentRow = memoSheet.addRow([`DATE OF ASSESSMENT: ${metrics.dateStr}`]);
    assessmentRow.getCell(1).font = { bold: true, size: 10, name: "Segoe UI" };

    const assessorRow = memoSheet.addRow(["ASSESSOR / WORKER: ShiftFlow Local Node Verification"]);
    assessorRow.getCell(1).font = { bold: true, size: 10, name: "Segoe UI" };

    const targetRow = memoSheet.addRow(["TARGET DISTRIBUTION: Executive Leadership Review"]);
    targetRow.getCell(1).font = { bold: true, size: 10, name: "Segoe UI" };

    memoSheet.addRow([]);

    const generalMetricsHead = memoSheet.addRow(["1. GENERAL ENGAGEMENT METRICS"]);
    generalMetricsHead.getCell(1).font = { bold: true, size: 11, color: { argb: "0F172A" }, name: "Segoe UI" };

    memoSheet.addRow([`During this shift, a total cumulative duration of ${metrics.totalHours} hours was recorded locally.`]);
    memoSheet.addRow([`Direct workspace productivity measures represent ${metrics.workHours} hours of focused, goal-oriented deliverables.`]);
    memoSheet.addRow([`A resting timeline benchmark of ${metrics.breakHours} hours of planned intermission was logged.`]);

    memoSheet.addRow([]);

    const deliverablesHead = memoSheet.addRow(["2. DELIVERABLES & HIGH-IMPACT RECONCILIATIONS"]);
    deliverablesHead.getCell(1).font = { bold: true, size: 11, color: { argb: "0F172A" }, name: "Segoe UI" };

    memoSheet.addRow([`• Focus Project Matrix: Primary development hours converged on "${metrics.topProject}".`]);
    memoSheet.addRow([`• Timeline Block Analytics: ${metrics.activityCount} micro-milestones were recorded.`]);
    memoSheet.addRow([`• Peak Performance Delivery: The single longest engaged session was devoted to: `]);
    memoSheet.addRow([
    ...
    const integrityHead = memoSheet.addRow(["3. INTEGRITY & LOCAL-FIRST COMPLIANCE ATTESTATION"]);
    integrityHead.getCell(1).font = { bold: true, size: 11, color: { argb: "0F172A" }, name: "Segoe UI" };

    memoSheet.addRow(["The timeline outputs logged in Sheet 1 conform to local browser validation criteria."]);
    memoSheet.addRow(["All records are chronologically locked with auto-calculated durations."]);

    memoSheet.addRow([]);

    const catAllocationHead = memoSheet.addRow(["4. CATEGORY ALLOCATIONS — Activity Category Allocation Share"]);
    catAllocationHead.getCell(1).font = { bold: true, size: 11, color: { argb: "0F172A" }, name: "Segoe UI" };

    if (metrics.categories && metrics.categories.length > 0) {
      metrics.categories.forEach((cat) => {
        memoSheet.addRow([`• ${cat.name}: ${cat.minutes} mins (${cat.percentage}%)`]);
      });
    } else {
    ...
    const primaryMetricsHead = memoSheet.addRow(["Primary Operational Metrics Summary"]);
    primaryMetricsHead.getCell(1).font = { bold: true, size: 11, color: { argb: "0F172A" }, name: "Segoe UI" };

    memoSheet.addRow([`Total Cumulative Logged Duration: ${metrics.totalHours} Hours`]);
    memoSheet.addRow([`Net Engineering Focus Delivery: ${metrics.workHours} Hours`]);
    memoSheet.addRow([`Operational Break / Recharge Time: ${metrics.breakHours} Hours`]);
    memoSheet.addRow([`Timeline Segments Processed: ${metrics.activityCount} Activities`]);
    memoSheet.addRow([`Longest Standalone engaged activity: "${metrics.longestActivity.title}" (${metrics.longestActivity.duration} mins)`]);
    memoSheet.addRow([`Primary Target Scope Focus: ${metrics.topProject}`]);

    memoSheet.addRow([]);

    const conclusionHead = memoSheet.addRow(["5. EXECUTIVE SUMMARY CONCLUSION"]);
    conclusionHead.getCell(1).font = { bold: true, size: 11, color: { argb: "0F172A" }, name: "Segoe UI" };

    const productivePct = metrics.totalMinutes > 0 ? Math.round((metrics.workMinutes / metrics.totalMinutes) * 100) : 0;
    const topCat = metrics.categories && metrics.categories[0] ? metrics.categories[0] : null;
    const conclusions = [];
    conclusions.push(
    ...
    if (topCat)
      conclusions.push(`Dominant activity category: ${topCat.name} — ${topCat.percentage}% (${topCat.minutes} mins).`);
    conclusions.push(
    ...
    if (productivePct >= 70) {
      conclusions.push("Recommendation: Sustain current workflows; formalize peak windows for planning and collaboration.");
    } else if (productivePct >= 45) {
      conclusions.push("Recommendation: Protect 1–2 uninterrupted deep-focus blocks per shift to improve delivery cadence.");
    } else {
      conclusions.push("Recommendation: Reduce context switching; re-balance tasks and introduce scheduled deep-work periods.");
    }
    ...
    if (hourlyImageId) {
      try {
        const hourlyChartSheet = workbook.addWorksheet("6. Hourly Activity Chart");
        hourlyChartSheet.views = [{ showGridLines: false }];
        hourlyChartSheet.addImage(hourlyImageId, { tl: { col: 1, row: 1 }, ext: { width: 850, height: 320 } });
      } catch (err) {
  ...
  const generatePDF = () => {
    ...
    doc.setFontSize(10);
    doc.setTextColor(textGray[0], textGray[1], textGray[2]);
    doc.text(`Operational Assessment Date: ${metrics.dateStr}  |  Compliance Engine: Local Node Verification`, 15, 34);
    ...
    doc.setFontSize(12);
    doc.setTextColor(slateDark[0], slateDark[1], slateDark[2]);
    doc.text("3. Category Allocations — Activity Category Allocation Share", 15, y);
    y += 6;
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9);
    doc.setTextColor(textGray[0], textGray[1], textGray[2]);
    ...
    (metrics.categories || []).slice(0, 8).forEach((cat, idx) => {
      ...
      doc.text(`${cat.name} — ${cat.percentage}% (${cat.minutes} mins)`, barX, rowY);
    ...
    if (productivePct >= 70) {
      conclusionParts.push("Recommend: Sustain current workflows; consider formalizing peak windows for collaborative planning.");
    } else if (productivePct >= 45) {
      conclusionParts.push("Recommend: Increase focus on priority deliverables by protecting 1-2 uninterrupted blocks per shift.");
    } else {
      conclusionParts.push("Recommend: Review task distribution and reduce context switching; schedule dedicated deep-focus blocks.");
    }
    ...
    doc.save(`ShiftFlow_Executive_Workspace_Report_${new Date().toISOString().slice(0, 10)}.pdf`);
  };

  const createLocalBackup = () => {
    const backupData = JSON.stringify(activities, null, 2);
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div>
        <h1 className="text-xl font-bold tracking-tight text-slate-850 dark:text-slate-100">Executive Portability</h1>
        <p className="text-xs text-slate-500 dark:text-slate-400">
          Generate clean reports or secure local data backups completely client-side.
        </p>
      </div>
      ...
      <div className="p-6 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-900 shadow-sm space-y-4 max-w-4xl">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Select a preset range or configure custom boundaries. Exports will respect this range.
            </p>
          </div>
        ...
        <div className="flex items-center justify-between pt-2 text-xs">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-slate-600 dark:text-slate-400 font-medium">Matching segments to export:</span>
            <strong className="font-mono text-brand-600 dark:text-brand-400 text-sm">{filteredActivities.length}</strong>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-4xl">
        <div className="card-lift p-5 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-900 shadow-sm flex items-center justify-between hover:border-brand-500/40 transition-all">
          <div>
            <h3 className="text-sm font-bold text-slate-850 dark:text-slate-200">Export Structured Excel Sheet</h3>
            <p className="text-xs text-slate-500 mt-1 font-sans">
              Sleek multi-sheet workbook including metrics summaries, timeline raw grids with Dates, category
              allocation share bars, and a high-resolution hourly line chart.
            </p>
          </div>
          <button
        ...
        <div className="card-lift p-5 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-900 shadow-sm flex items-center justify-between hover:border-brand-500/40 transition-all">
          <div>
            <h3 className="text-sm font-bold text-slate-850 dark:text-slate-200">Export Executive PDF Dossier</h3>
            <p className="text-xs text-slate-500 mt-1">
              Stunning, multi-page vector PDF with KPI widgets, category share bar graphics, and page-wrapped
              timeline trees.
            </p>
          </div>
          <button
        ...
        <div className="card-lift p-5 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-900 shadow-sm flex items-center justify-between hover:border-brand-500/40 transition-all">
          <div>
            <h3 className="text-sm font-bold text-slate-850 dark:text-slate-200">Export Raw CSV Log</h3>
            <p className="text-xs text-slate-500 mt-1">Generic plain-text schema optimized for universal compatibility.</p>
          </div>
          <button
            onClick={generateCSV}
            disabled={filteredActivities.length === 0}
        ...
        <div className="card-lift p-5 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-900 shadow-sm flex items-center justify-between hover:border-brand-500/40 transition-all">
          <div>
            <h3 className="text-sm font-bold text-slate-850 dark:text-slate-200">Backup IndexedDB Ledger</h3>
            <p className="text-xs text-slate-500 mt-1">Complete system snapshotted down into portable JSON arrays.</p>
          </div>
          <button
        ...
        <div className="p-5 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-900 shadow-sm col-span-1 md:col-span-2 space-y-3">
          <h3 className="text-sm font-bold text-slate-850 dark:text-slate-200">Restore/Import System Backups</h3>
          <p className="text-xs text-slate-500">Restore dynamic snapshots back into IndexedDB. Note: This clears previous logs.</p>
          <label className="inline-block px-4 py-2 bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-850 border border-slate-200 dark:border-slate-800 text-xs font-semibold rounded-lg text-slate-700 dark:text-slate-300 cursor-pointer transition-all">
            ...
            <input type="file" accept=".json" onChange={restoreLocalBackup} className="hidden" />
          </label>
        </div>
      </div>
    </div>
