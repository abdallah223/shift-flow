...
export default function SettingsView() {
  const { categories, setCategories, workHoursGoal, updateWorkHoursGoal } = useApp();
  ...
  const handleDeleteCategory = async (id) => {
    await dbInstance.delete("categories", id);
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div>
        <h1 className="text-xl font-bold tracking-tight text-slate-850 dark:text-slate-100">Workspace Controls</h1>
        ...
        <div className="p-6 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-900 shadow-sm space-y-4">
          ...
          <div className="space-y-1.5 max-h-64 overflow-y-auto pr-1">
            {categories.map((cat) => (
              <div
                key={cat.id}
                className="flex items-center justify-between p-2.5 rounded-lg bg-slate-50 dark:bg-slate-950/40 border border-slate-150 dark:border-slate-900/60 text-xs"
              >
        ...
        <div className="p-6 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-900 shadow-sm space-y-4 h-fit">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-805 rounded-lg p-2.5 text-slate-805 dark:text-slate-205 text-xs focus:outline-none font-mono"
            />
          </div>
          ...
          <div className="space-y-3 pt-3 border-t border-slate-200 dark:border-slate-800">
            ...
            <div className="space-y-2">
              {[
                ...
                { key: "E", desc: "Jump directly to Executive reports" },
                { key: "Ctrl + K", desc: "Toggle visual command palette search" },
              ].map((sh, index) => (
                <div key={index} className="flex items-center justify-between text-[11px]">
                  <span className="text-slate-500 font-medium">{sh.desc}</span>
                  <kbd className="px-1.5 py-0.5 bg-slate-105 dark:bg-slate-950 border border-slate-200 dark:border-slate-805 text-slate-600 dark:text-slate-400 font-mono rounded text-[10px] uppercase">
                    {sh.key}
                  </kbd>
                </div>
              ))}
